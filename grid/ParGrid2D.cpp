...
ParGrid2D::ParGrid2D(const Teuchos::RCP<Grid2D> &grid, const Teuchos::RCP<Teuchos::MpiComm<int>>& tcomm) : m_comm(tcomm)
{
  // initilization
  //@{
  idx_t *elmwgt = 0;
  ...
  //@{
  int size, rank;
  MPI_Comm comm = getRawMpiComm(*tcomm);
  rank = tcomm->getRank();
  size = tcomm->getSize();
  //@}
  ...
  int nelemperp = std::ceil(grid->m_elems.size()/(double)size);
  int count = 0;
  int toomuch = nelemperp*size - grid->m_elems.size();
  ...
  for (int proc = 0; proc < size; proc++) 
  {
    ...
    count += nelemperp;
    if (count >= int(grid->m_elems.size()))
      count = grid->m_elems.size();
  }
  
  elmdist[size] = grid->m_elems.size();
  int nparts = size;
  //@}
  ...
  for (int i = 0; i < n_elems_init_loc; i++)
    tot_num_points += grid->m_elems[elmdist[rank]+i]->m_nodes.size();
  
  idx_t eptr [n_elems_init_loc+1];
  idx_t eind [tot_num_points];
  for (int i = 0; i < n_elems_init_loc; i++)
  {
    int loc_num_points = grid->m_elems[elmdist[rank]+i]->m_nodes.size();
    eptr[i] = i*loc_num_points;
    for (int point_ind (0); point_ind < loc_num_points; ++ point_ind)
      eind[i*loc_num_points+point_ind] = grid->m_elems[elmdist[rank]+i]->m_nodes[point_ind];
  }
  ...
  for (int i = 0; i < _n_elems_local; i++) 
  {
    ...
    for (std::size_t k (0); k < grid->m_elems[iglob]->m_nodes.size(); ++k)
    {
      int pointInd = grid->m_elems[iglob]->m_nodes[k];
      ...
      auto& node_stencil_ = grid->m_stencil[pointInd];
  ...
  // get the global vertices of the grid
  m_points.resize(grid->m_points.size());
  for (int i = 0; i < int(grid->m_points.size()); i++)
  {
    m_points[i] = grid->m_points[i];
  }
  ...
  for (int i = 0; i < _n_elems; i++)
  {
    ...
    if (grid->m_elems[icell]->GetElementType() == typeTriangle)
      m_elems[i] = new Triangle(grid->m_elems[icell]->m_nodes[0],grid->m_elems[icell]->m_nodes[1],grid->m_elems[icell]->m_nodes[2]);
    else 
      m_elems[i] = new Quadrilateral(grid->m_elems[icell]->m_nodes[0],grid->m_elems[icell]->m_nodes[1],grid->m_elems[icell]->m_nodes[2],grid->m_elems[icell]->m_nodes[3]);
    
    m_elems[i]->area = grid->m_elems[icell]->area;
    m_elems[i]->orientation = grid->m_elems[icell]->orientation;
  }
  ...
  {
    for (auto it = grid->m_stencil[m_points_ind_loc[i]].begin(); it != grid->m_stencil[m_points_ind_loc[i]].end(); ++it)
    {
  ...
  for (int i = 0; i < _n_elems_local; i++)
  {
    ...
    if (grid->m_elems[icell]->GetElementType()==typeQuadrilateral)
      m_neighbors[i].resize(4);
    else 
      m_neighbors[i].resize(3);
    
    if ( grid->m_neighbors[icell][0] >= 0)
    {
      m_neighbors[i][0]=glob2loc[grid->m_neighbors[icell][0]];
    } else {
      m_neighbors[i][0]=grid->m_neighbors[icell][0];
    }
    
    if ( grid->m_neighbors[icell][1]>=0 ) 
    {
      m_neighbors[i][1]=glob2loc[grid->m_neighbors[icell][1]];
    } else {
      m_neighbors[i][1]=grid->m_neighbors[icell][1];
    }
    
    if (grid->m_neighbors[icell][2]>=0)
    {
      m_neighbors[i][2]=glob2loc[grid->m_neighbors[icell][2]];
    } else {
      m_neighbors[i][2]=grid->m_neighbors[icell][2];
    }
    
    if (grid->m_elems[icell]->GetElementType()==typeQuadrilateral)
    {
      if (grid->m_neighbors[icell][3]>=0)
      {
        m_neighbors[i][3]=glob2loc[grid->m_neighbors[icell][3]];
      } else {
        m_neighbors[i][3]=grid->m_neighbors[icell][3];
      }      
    }
  }
...
const std::vector<Point2D> ParGrid2D::getElementVertices(const int elem_ind) const
{
  if (m_elems.at(elem_ind)->GetElementType() == typeTriangle)
    ...
    throw("Error in ParGrid2D::getElementVertices!! Element type not valid.");
}

///////////////////////////////////////////////////////////////////////////////////////////////////////

const std::vector<Point2D> ParGrid2D::getEdgeVertices(const int edge_ind) const
{
...
void ParGrid2D::setEdges()
{
  for (unsigned int ElemInd(0); ElemInd < m_elems.size(); ++ElemInd)
  {
    ...
    {
      std::cout << "In ParGrid2D::SetEdges : unsupported element type!!!\n";
      exit(1);
    }
  }
